...
check <- WholePop %>% 
  ...
  mutate(ShapeGroup = as.factor(ShapeGroup)) %>% 
  ggplot(.)+
  geom_point(aes(x=Year, y=Total, group=Status, colour=ColourGroup, fill=ColourGroup,  shape=ShapeGroup), size=2.5)+
  geom_line(aes(x=Year, y=Total, group=Status, colour=ColourGroup))+
  theme_classic()+
  geom_vline(xintercept=6.6, linetype="dashed", color="grey20")+
  geom_vline(xintercept=10, colour="grey20")+
  geom_vline(xintercept=12.5, linetype="dashed", colour="grey20")+
  #geom_vline(xintercept=10.6, linetype="dotted", colour="grey20")+
  scale_shape_manual(values= c(`Pre-1987`="circle",`NTZ.Normal`="square filled", `Pre-1987`="circle",
                               `Fished.Normal`="square open"), name="Area and scenario",
                     labels=c("Pre-1987", "Normal Scenario (NTZ)",
                              "Normal Scenario (Fished)")
                     , guide="none")+
  ylab(NULL)+
  xlab(NULL)
#ggplot2::annotate("text", x=1.7, y=0.016, label="(c) Legal sized", size = 3, fontface=2)
